...
(The idea being something like [[https://github.com/dylanaraps/neofetch][neofetch]] or [[https://github.com/fastfetch-cli/fastfetch][fastfetch]], but for/in Emacs; but also (perhaps more usefully) a nice frontend for programmatically executing Elisp [in your ~init.el~, do different things depending not only on whether you're on Linux or another OS, but also depending on exactly which Linux distro (or distro family) you're running on] or doing other system-specific things [e.g., setting PDF metadata].)
