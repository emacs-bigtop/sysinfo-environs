...
The beginnings of a neofetch/fastfetch function are to be found by calling ~sysinfo-environs-neofetch-like~.
...
But in this case what appears to be the right logo icon is actually located at =/usr/share/pixmaps/fedora-logo-sprite.svg=. (currently in an unpushed branch, we currently just have a hack to find the right icon for secureblue....)
